...
(function($){
	...
	var methods = {
		init : function(config) {
			...
			var width = (options.useWindowWidth === true) ? x : screen.width;
			if (width < options.deviceWidth){
				var _o = $(this), // store the jqyuery object once
					...
					$("<option />", {
						 "selected": (!$('.active-trail', _o).length) ? 'selected' : false,
						 "value": "",
						 "text": options.defaultOption
					}).appendTo(_s);

					// Populate the dropdown with menu items. If there is an li.active-trail we'll
					// make this one selected
					$('a', _o).each(function() {
						var el = $(this),
						sl = ( el.parent('li').hasClass('active-trail') || el.hasClass('active-trail') ) ? 'selected' : false;
						$("<option />", {
							"selected": sl,
							"value": el.attr("href"),
							"text": el.text()
